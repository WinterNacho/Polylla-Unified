...
#ifndef POLYLLA_GPU_CU
#define POLYLLA_GPU_CU
...
public:
    ...
    //Constructor from a node_file and ele_file (without neigh_file)
    ...
    void construct_Polylla(){
        ...
        // Handle triangle regions for GPU
        int *triangle_regions_d = nullptr;
        int *triangle_regions_h = nullptr;
        if (use_regions && mesh_input->triangle_regions.size() > 0) {
            triangle_regions_h = mesh_input->triangle_regions.data();
            cudaMalloc(&triangle_regions_d, n_triangle*sizeof(int));
        }
        ...
        // Copy triangle regions if using regions
        if (use_regions && triangle_regions_d != nullptr) {
            cudaMemcpy(triangle_regions_d, triangle_regions_h, n_triangle*sizeof(int), cudaMemcpyHostToDevice);
        }
        ...
        gpuErrchk( cudaDeviceSynchronize() );
        t_start = std::chrono::high_resolution_clock::now();
        label_phase<<<(n_halfedges + BSIZE - 1)/BSIZE, BSIZE>>>(halfedges_d, max_edges_d, frontier_edges_d, triangle_regions_d, n_triangle, n_halfedges); 
        ...
        seed_phase_d<<<(n_halfedges + BSIZE - 1)/BSIZE, BSIZE>>>(halfedges_d, max_edges_d, seed_edges_ad, triangle_regions_d, n_triangle, n_halfedges); 
        gpuErrchk( cudaDeviceSynchronize() );
       ...
       //print_all_halfedges<<<(n_halfedges + BSIZE - 1)/BSIZE, BSIZE>>>(halfedges_d, n_halfedges);
        ...
        cudaFree(seed_edges_ad);
        cudaFree(seed_edges_bd);
        cudaFree(halfedges_d);
        cudaFree(vertices_d);
        cudaFree(output_HalfEdges_d);
        
        // Clean up triangle regions memory if allocated
...
public:
    ...
    // Getters and setters for region support
