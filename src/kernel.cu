...
// Device functions for region support
__device__ int index_face_d(int e) {
    return e / 3;
}

__device__ int region_face_d(int *triangle_regions, int f, int n_faces) {
    if (triangle_regions != nullptr && f < n_faces && f >= 0)
        return triangle_regions[f];
    return 0;
}

__device__ bool is_region_boundary_d(int *triangle_regions, halfEdge *HalfEdges, int e, int n_faces) {
    if (triangle_regions == nullptr) return false;
    
    int twin = twin_d(HalfEdges, e);
    int region1 = region_face_d(triangle_regions, index_face_d(e), n_faces);
    int region2 = region_face_d(triangle_regions, index_face_d(twin), n_faces);
...
__device__ bool is_frontier_edge_d(halfEdge *halfedges, bit_vector_d *max_edges, int *triangle_regions, int n_faces, const int e)
{
    ...
    bool is_region_boundary = is_region_boundary_d(triangle_regions, halfedges, e, n_faces);
    
    return is_border_edge || is_not_max_edge || is_region_boundary;
}

__global__ void label_phase(halfEdge *halfedges, bit_vector_d *max_edges, bit_vector_d *frontier_edges, int *triangle_regions, int n_faces, int n){
    int off = threadIdx.x + blockDim.x*blockIdx.x;
    if (off < n){
        frontier_edges[off] = 0;
        if(is_frontier_edge_d(halfedges, max_edges, triangle_regions, n_faces, off))
...
__device__ bool is_seed_edge_d(halfEdge *HalfEdges, bit_vector_d *max_edges, int *triangle_regions, int n_faces, int e){
    int twin = twin_d(HalfEdges, e);
    ...
    bool is_terminal_region_edge = is_region_boundary_d(triangle_regions, HalfEdges, e, n_faces) && max_edges[e];
...
__global__ void seed_phase_d(halfEdge *HalfEdges, bit_vector_d *max_edges, half *seed_edges, int *triangle_regions, int n_faces, int n){
